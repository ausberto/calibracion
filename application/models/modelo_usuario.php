...
class Modelo_usuario extends CI_Model {
    ...
    function Insert($NombreUsuario, $Clave, $TipoUsuario) {
        $Activo = "S";
        $sql = "INSERT INTO usuario (CodPersona, NombreUsuario, Clave,TipoUsuario,Activo) VALUES 
    ...
    function Update($CodUsuario, $CodPersona, $NombreUsuario, $Clave, $TipoUsuario) {
        $sql = "UPDATE $this->Tabla SET  NombreUsuario='$NombreUsuario',Clave='$Clave',TipoUsuario='$TipoUsuario'
    ...
    function GetAll() {
        $sql = "SELECT u.*,p.*,pr.Nombres,pr.Paterno,pr.Materno FROM  usuario u, perfil p,persona pr 
		        WHERE pr.CodPersona=u.CodPersona and u.TipoUsuario=p.CodPerfil and u.Activo in ('S')";
