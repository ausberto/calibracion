...
CREATE  TABLE IF NOT EXISTS `upea_matriculacion`.`matricula_postgrado` (
  ...
  INDEX `fk_matricula_postgrado_2_idx` () ,
  ...
  CONSTRAINT `fk_matricula_postgrado_2`
    FOREIGN KEY ()
    REFERENCES `upea_matriculacion`.`planificacion_postgrado` ()
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;
...
CREATE  TABLE IF NOT EXISTS `upea_matriculacion`.`usuario` (
  ...
  `TipoUsuario` VARCHAR(1) NULL DEFAULT NULL ,
  `Activo` VARCHAR(1) NULL DEFAULT 'S' ,
